...
export const RecentActivityTicker = ({ activities }: { activities: ActivityItem[] }) => {
  if (activities.length === 0) return null;

  // Limit to 10 recent items
  const recent10 = activities.slice(0, 10);

  // Always scroll if there are items, to keep the ticker behavior consistent
  const needsScroll = recent10.length > 0;
    ...
    <div className="flex-1 mx-6 flex flex-col gap-1.5">
      ...
      <div className="h-[84px] bg-stone-50 rounded-xl border border-stone-200 overflow-hidden relative flex items-center px-4 shadow-inner">
        <div className="relative h-full flex-1 overflow-hidden">
          <motion.div
            key={activities[0]?.timeString || 'empty'}
            animate={needsScroll ? { y: [0, -scrollDistance] } : { y: 0 }}
            transition={needsScroll ? { 
              duration: scrollDistance / 15, // 15px per second, slower readable speed
              ease: "linear",
              repeat: Infinity,
              delay: 5 // Pause for 5 seconds before starting to scroll
            } : {}}
